...
const FrameworkListPage = () => {
  const [pageNumber, setPageNumber] = useState(1);
  const [perPage, setPerPage] = useState(5);
  const [searchKey, setSearchKey] = useState(0);

  useEffect(() => {
    FrameworkRequest.FrameworkList(pageNumber, perPage, searchKey);
  }, [pageNumber, perPage, searchKey]);

  const { FrameworkLists, TotalFramework } = useSelector((state) => state.Framework);

  const PerPageOnChange = (e) => {
    if (e.target.value === "All") {
      setPerPage(TotalFramework);
    } else {
  ...
  const DeleteFramework = (id) => {
    AleartMessage.Delete(id, FrameworkRequest.FrameworkDelete).then((result) => {
      if (result) {
        FrameworkRequest.FrameworkList(pageNumber, perPage, searchKey);
      }
    });
            ...
            <nav aria-label="breadcrumb">
              <ul className="breadcrumb">
                <li className="breadcrumb-item active" aria-current="page">
                  ...
                  <PageTitle
        breadCrumbItems={[
          { label: "Framework", path: "/framework/ramework-list" },
          {
            label: "Create List",
            ...
            <Card.Body>
              <Row className="mb-2">
                <Col sm={5}>
                  ...
                  >
                    <i className="mdi mdi-plus-circle me-2"></i> Add Framework
                  </Link>
                </Col>
                ...
                <Col>
                  <div className="mb-2">
                    <span className="d-flex align-items-center">
                      Search :{" "}
                      <input
                        placeholder={TotalFramework + " records..."}
              ...
              <Row>
                <Col>
                  <Table className="table-centered react-table" responsive>
                    ...
                    <tbody>
                      {FrameworkLists?.map((record, index) => {
                        return (
                          <tr key={index}>
                            <td>{record?.name}</td>
                            ...
                            <td>
                              <Link
                                to={`/Framework/Framework-create-update?id=${record?.id}`}
                ...
                <Col>
                  <div className="d-lg-flex align-items-center text-center pb-1">
                    ...
                    <span className="me-3">
                      Page
                      <strong>
                        {pageNumber} of {Math.ceil(TotalFramework / perPage)}
                      </strong>
                    </span>
                    ...
                    <ReactPaginate
                      ...
                      pageCount={TotalFramework / perPage}
...
export default FrameworkListPage;
