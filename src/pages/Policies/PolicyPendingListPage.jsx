...
const PolicyPendingListPage = () => {
  ...
  useEffect(() => {
    ...
    PolicyRequest.PolicyPendingList(id,UserDetails.id,pageNumber, perPage, searchKey);
  }, [pageNumber, perPage, searchKey]);

  const { PolicyPendingList, TotalPolicy } = useSelector((state) => state.Policy);
              ...
              <Row>
                <Col>
                  <Table className="table-centered react-table" responsive>
                    ...
                    <tbody>
                      {PolicyPendingList?.map((record, index) => {
                        return (
                          <tr key={index}>
...
export default PolicyPendingListPage;
