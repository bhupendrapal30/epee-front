...
const PolicyRejectListPage = () => {
  ...
  useEffect(() => {
    ...
    PolicyRequest.PolicyRejectList(id,UserDetails.id,pageNumber, perPage, searchKey);
  }, [pageNumber, perPage, searchKey]);

  const { PolicyRejectList, TotalPolicy } = useSelector((state) => state.Policy);
              ...
              <Row>
                <Col>
                  <Table className="table-centered react-table" responsive>
                    ...
                    <tbody>
                      {PolicyRejectList?.map((record, index) => {
                        return (
                          <tr key={index}>
...
export default PolicyRejectListPage;
