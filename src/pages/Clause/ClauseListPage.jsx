...
const ClauseListPage = () => {
  ...
  useEffect(() => {
    ClauseRequest.ClauseList(pageNumber, perPage, searchKey);
  }, [pageNumber, perPage, searchKey]);

  const { ClauseLists, TotalClause } = useSelector((state) => state.Clause);

  const PerPageOnChange = (e) => {
    if (e.target.value === "All") {
      setPerPage(TotalClause);
    } else {
  ...
  const DeleteClause = (id) => {
    AleartMessage.Delete(id, ClauseRequest.ClauseDelete).then((result) => {
      if (result) {
        ClauseRequest.ClauseList(pageNumber, perPage, searchKey);
      }
    });
          ...
          <Card>
            <Card.Body>
              <Row className="mb-2">
                <Col sm={5}>
                  <Link
                    to="/clause/clause-create-update"
                    className="btn btn-danger mb-2"
                  >
                    <i className="mdi mdi-plus-circle me-2"></i> Add Clause
                  </Link>
                </Col>
                ...
                <Col>
                  <div className="mb-2">
                    <span className="d-flex align-items-center">
                      Search :{" "}
                      <input
                        placeholder={TotalClause + " records..."}
                      ...
                      {ClauseLists?.map((record, index) => {
                        return (
                          <tr key={index}>
                            ...
                            <td>
                              <Link
                                to={`/clause/clause-create-update?id=${record?.id}`}
                ...
                <Col>
                  <div className="d-lg-flex align-items-center text-center pb-1">
                    ...
                    <span className="me-3">
                      Page
                      <strong>
                        {pageNumber} of {Math.ceil(TotalClause / perPage)}
                      </strong>
                    </span>
