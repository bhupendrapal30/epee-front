...
const PermissionListPage = () => {
  ...
  useEffect(() => {
    PermissionRequest.PermissionList(pageNumber, perPage, searchKey);
  }, [pageNumber, perPage, searchKey]);

  
  const { PermissionLists, TotalPermission } = useSelector((state) => state.Permission);
   ...
   if (UserDetails.per.length > 0) {
    let perDataM= UserDetails.per;
      for (let index in perDataM) {
         if(perDataM[index].modulesname=='permission'){
            perMissArr =1
         }
      }
   }
  ...
  const DeletePermission = (id) => {
    AleartMessage.Delete(id, PermissionRequest.PermissionDelete).then((result) => {
      if (result) {
        PermissionRequest.PermissionList(pageNumber, perPage, searchKey);
      }
    });
                ...
                <Col>
                  <div className="mb-2">
                    <span className="d-flex align-items-center">
                      Search :{" "}
                      <input
                        placeholder={TotalPermission + " records..."}
                ...
                <Col>
                  <Table className="table-centered react-table" responsive  >
                    ...
                    <tbody>
                      ...
                      {PermissionLists?.map((record, index) => {
                        return (
                          <tr key={index}  test={record}>
                            <td>{record?.rolename}</td>
                            <td>{record?.modulesname}</td>
                            <td>{record?.addedit==1?"Yes":"No"}</td>
                            <td>{record?.view==1?"Yes":"No"}</td>
                            <td>{record?.deleteflag==1?"Yes":"No"}</td>
                            ...
                            <td>
                              <Link
                                to={`/permission/permission-create-update?id=${record?.id}`}
                ...
                <Col>
                  <div className="d-lg-flex align-items-center text-center pb-1">
                    ...
                    <span className="me-3">
                      Page
                      <strong>
                        {pageNumber} of {Math.ceil(TotalPermission / perPage)}
                      </strong>
                    </span>
                    ...
                    <ReactPaginate
                      ...
                      pageCount={TotalPermission / perPage}
