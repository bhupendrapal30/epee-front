...
const SubcontrolListPage = () => {
  const [pageNumber, setPageNumber] = useState(1);
  const [perPage, setPerPage] = useState(5);
  const [searchKey, setSearchKey] = useState(0);

  useEffect(() => {
    SubcontrolRequest.SubcontrolList(pageNumber, perPage, searchKey);
  }, [pageNumber, perPage, searchKey]);

  const { SubcontrolLists, TotalSubcontrol } = useSelector((state) => state.Subcontrol);

  const PerPageOnChange = (e) => {
    if (e.target.value === "All") {
      setPerPage(TotalSubcontrol);
    } else {
  ...
  const DeleteSubcontrol = (id) => {
    AleartMessage.Delete(id, SubcontrolRequest.SubcontrolDelete).then((result) => {
      if (result) {
        SubcontrolRequest.SubcontrolList(pageNumber, perPage, searchKey);
      }
    });
            ...
            <Card.Body>
              <Row className="mb-2">
                <Col sm={5}>
                  ...
                  >
                    <i className="mdi mdi-plus-circle me-2"></i> Add Subcontrol
                  </Link>
                </Col>
                ...
                <Col>
                  <div className="mb-2">
                    <span className="d-flex align-items-center">
                      Search :{" "}
                      <input
                        placeholder={TotalSubcontrol + " records..."}
                  ...
                  <Table className="table-centered react-table" responsive>
                    ...
                    >
                      <tr>
                        ...
                        <th>Framework</th>
                        <th>Created On</th>
                        <th>Subcontrol Status</th>
                        <th>Action</th>
                      </tr>
                    </thead>
                    <tbody>
                      {SubcontrolLists?.map((record, index) => {
                        return (
                          <tr key={index}>
                            <td>{record?.name}</td>
                            <td>{record?.control_id}</td>                            <td>{record?.frameworkid}</td>
                            ...
                            <td>
                              <Link
                                to={`/Subcontrol/Subcontrol-create-update?id=${record?.id}`}
                ...
                <Col>
                  <div className="d-lg-flex align-items-center text-center pb-1">
                    ...
                    <span className="me-3">
                      Page
                      <strong>
                        {pageNumber} of {Math.ceil(TotalSubcontrol / perPage)}
                      </strong>
                    </span>
                    ...
                    <ReactPaginate
                      ...
                      pageCount={TotalSubcontrol / perPage}
...
export default SubcontrolListPage;
